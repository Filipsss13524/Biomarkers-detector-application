...
class _ResultScreenState extends State<ResultScreen> {
  ...
  Future<Data> _sendRequest(String name, String surname, String date, String gender, String filePath) async {
    // ngrok server routing method

    if (widget.currentChosenAddressType == 0) {
      setState(() {
        //global host server
        imageUrl = 'https://${widget.currentDomain}.ngrok-free.app/api_image';
        recordUrl = 'https://${widget.currentDomain}.ngrok-free.app/upload';
      });
    } else if (widget.currentChosenAddressType == 1) {
      setState(() {
        imageUrl = 'https://${widget.currentngrokAddress}-${widget.currentngrokAddressIp}.ngrok-free.app/api_image';
        recordUrl = 'https://${widget.currentngrokAddress}-${widget.currentngrokAddressIp}.ngrok-free.app/upload';
      });
    } else {
      setState(() {
        // local host server ip 
        imageUrl = 'http://${widget.currentAddress}:${widget.currentPort}/api_image';
        recordUrl = 'http://${widget.currentAddress}:${widget.currentPort}/upload';
      });
    }
    // test servers
    // const imageUrl  = 'https://9837-178-159-129-139.ngrok-free.app/api_image';
    // const recordUrl = 'https://9837-178-159-129-139.ngrok-free.app/upload';
    ...
    request.fields['text'] = '$name-$surname-$date-$gender-${DateFormat('yyyy-MM-dd-HH-mm').format(now)}';
    ...
    final imageResponse = await http.post(
      ...
      body: jsonEncode({'query': '$name-$surname-$date-$gender-${DateFormat('yyyy-MM-dd-HH-mm').format(now)}'}),
    );
  ...
  Widget build(BuildContext context) {
    ...
    diagnosis = {'PD': 'Parkinson Disease', 'HC': 'Health Control'};
